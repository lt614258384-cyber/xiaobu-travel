...
def seed():
    ...
    for region_data in REGIONS:
        ...
        n = len(loc_names)
        # Build mesh: each location connects to 3-5 neighbors within region
        # Ring + cross-connections forming a connected web
        mesh_adj = {name: set() for name in loc_names}
        for i in range(n):
            # Ring connections: i-2, i-1, i+1, i+2 (wrapping around)
            for offset in [-2, -1, 1, 2]:
                j = (i + offset) % n
                if j != i:
                    mesh_adj[loc_names[i]].add(loc_names[j])

        # Add 2-4 extra random cross-connections per location for density
        for i in range(n):
            pool = [name for j, name in enumerate(loc_names) if j != i and name not in mesh_adj[loc_names[i]]]
            extra = _rng.sample(pool, min(_rng.randint(2, 4), len(pool)))
            for name in extra:
                mesh_adj[loc_names[i]].add(name)
                mesh_adj[name].add(loc_names[i])

        # Create locations with mesh adjacency
        for name, desc, atmosphere in region_data["locations"]:
            loc = Location(
                ...
                adjacent_locations=sorted(mesh_adj[name]),  # resolved to IDs below
            )
            session.add(loc)
            session.flush()
    ...
    for region_data in REGIONS:
        loc_names = [l[0] for l in region_data["locations"]]
        for name, desc, atmosphere in region_data["locations"]:
            adj_ids = [location_map[n].id for n in mesh_adj[name] if n in location_map]
